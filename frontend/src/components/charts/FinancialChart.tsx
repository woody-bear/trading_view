...
export default function FinancialChart({ symbol, market }: Props) {
  ...
  return (
    <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-4 mt-4 mb-4">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-1 bg-[var(--bg)] rounded-md p-0.5">
          <button onClick={() => setPeriod('annual')}
            className={`px-2.5 py-1 rounded text-caption md:text-caption font-semibold transition ${period === 'annual' ? 'bg-[var(--card)] text-white' : 'text-[var(--muted)]'}`}>
            연간
          </button>
          <button onClick={() => setPeriod('quarterly')}
            className={`px-2.5 py-1 rounded text-caption md:text-caption font-semibold transition ${period === 'quarterly' ? 'bg-[var(--card)] text-white' : 'text-[var(--muted)]'}`}>
            분기
          </button>
        </div>
      </div>
          ...
          return (
            <div key={d[labelKey]} className="flex-1 flex flex-col items-center justify-end h-full">
              ...
              <div className="text-center mt-1.5">
                <div className="text-caption md:text-caption text-white font-semibold">
                  {period === 'annual' ? d.year : d.quarter?.split('-')[1]}
                </div>
                {change != null && (
                  <div className={`text-caption md:text-micro font-mono ${change >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                    {change >= 0 ? '+' : ''}{change}%
                  </div>
                )}
              </div>
        ...
        {items.map((d: any) => {
          ...
          return (
            <div key={d[labelKey]} className="flex-1 text-center">
              <div className="text-caption md:text-micro text-[var(--muted)] font-mono">
      ...
      {/* 단위 */}
      <div className="text-micro text-[var(--muted)] text-right mt-2 opacity-60">
