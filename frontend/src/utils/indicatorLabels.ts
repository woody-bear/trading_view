...
export function signalStrengthBadge(state: string, grade: string): Badge | null {
  if (state === 'BUY' && grade === 'STRONG')
    return { label: 'STRONG BUY', cls: 'text-green-400 bg-green-400/15 border border-green-400/30', priority: 1 }
  if (state === 'BUY' && grade === 'WEAK')
    return { label: 'WEAK BUY', cls: 'text-green-300 bg-green-300/10', priority: 1 }
  if (state === 'SELL' && grade === 'STRONG')
    return { label: 'STRONG SELL', cls: 'text-red-400 bg-red-400/15 border border-red-400/30', priority: 1 }
  if (state === 'SELL' && grade === 'WEAK')
    return { label: 'WEAK SELL', cls: 'text-red-400 bg-red-400/10', priority: 1 }
  return null
}
...
export function indicatorBadges(data: IndicatorData, maxCount = 4): Badge[] {
  ...
  if (sq >= 3) badges.push({ label: 'MAX SQ', cls: 'text-red-400 bg-red-400/10', priority: 10 })
  ...
  if (rsi !== undefined && rsi !== null) {
    ...
    else if (rsi > 70) badges.push({ label: 'RSI 과매수', cls: 'text-red-400 bg-red-400/10', priority: 21 })
